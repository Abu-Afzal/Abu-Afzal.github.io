...
document.addEventListener('DOMContentLoaded', () => {
    ...
    auth.onAuthStateChanged(async (user) => {
        if (!user) {
            // Tidak ada sesi Auth aktif → redirect ke login
            alert('Anda harus login terlebih dahulu!');
            window.location.href = '../home.html';
            return;
        }
        
        // ✅ Ambil data user dari Auth + localStorage (untuk nama, role, dll)
        let localUser = null;
        try {
        ...
        // Load daftar kelas dari SIPENA (Firestore via RTDB-style)
...
async function loadDaftarKelas() {
    try {
        ...
        // Baca dari Firestore collection 'sipena2' (format flat seperti di sipena-kelas.js)
...
async function simpanJurnal(e) {
    ...
    try {
        ...
        // ✅ Simpan ke Firestore (sekarang auth.currentUser sudah terisi → rules lolos)
