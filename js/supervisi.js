...
window.downloadPDF = async function(docId) {
  try {
    ...
    // A4 landscape supaya semua komponen cukup dalam 1 halaman
    ...
    if (components.length > 0) {
      ...
      const footerHeight = data.notes ? 24 : 16; // ruang untuk keterangan + ttd
      ...
      // Hitung row height agar semua muat
      const headerHeight = 8;
      const footerRowsHeight = 22; // 3 baris footer (jumlah, %, predikat)
      const rowHeight = Math.min(
        7, // max row height
        Math.max(5, (availableHeight - headerHeight - footerRowsHeight) / numRows)
      );

      // Kolom: No | Komponen | 1 | 2 | 3 | 4 | Skor
      const colNo     = 8;
      const colSkor   = 12;
      const colCheck  = 10;
      ...
      // Baris header 1 (merged header atas)
      ...
      // Baris header 2 (angka 1-4)
      pdf.setFillColor(129, 199, 132); pdf.setTextColor(0, 0, 0);
      pdf.rect(margin, y, tableWidth, headerHeight / 2, 'FD');
      pdf.text('', colX[0] + colWidths[0] / 2, y + 3, { align: 'center' });
      pdf.text('', colX[1] + colWidths[1] / 2, y + 3, { align: 'center' });
...
// Fungsi untuk print langsung hasil supervisi
      ...
      components.forEach((comp, index) => {
        ...
        htmlContent += `
          <tr>
            <td>${index + 1}</td>
            <td>${comp.name}</td>
            <td style="text-align: center; font-weight: bold;">${score}/4</td>
          </tr>
        `;
      });
    }
    ...
    htmlContent += `
          ...
          <tfoot>
            <tr class="summary">
              ...
              <td style="text-align: center;"><strong>${data.totalScore}/${data.maxScore}</strong></td>
            </tr>
            <tr class="summary">
              <td colspan="2" style="text-align: right;">Persentase</td>
              <td style="text-align: center;"><strong>${data.percentage}%</strong></td>
            </tr>
            <tr class="summary">
              <td colspan="2" style="text-align: right;">Predikat</td>
              <td style="text-align: center;"><strong>${data.predicate || '-'}</strong></td>
            </tr>
          </tfoot>
        </table>
        ...
        <div class="notes-section">
          <div class="notes-box">
            <h4 style="margin-top: 0; color: #1e40af;">Catatan Khusus:</h4>
            <p>${data.notes || '-'}</p>
          </div>
          <div class="notes-box">
            <h4 style="margin-top: 0; color: #1e40af;">Rencana Tindak Lanjut:</h4>
            <p>${data.actionPlan || '-'}</p>
          </div>
        </div>
