...
async function loadScheduleList() {
  ...
  try {
    ...
    container.innerHTML = schedules.map(s => {
      ...
      return `<div class="schedule-card"><div class="schedule-info"><div class="schedule-title">${roleIcon} ${s.teacherName}</div><div class="schedule-detail">📅 ${formatDate(s.scheduledDate)} | 👤 Supervisor: ${s.supervisorName} | ${statusBadge}</div>${s.notes ? `<div class="schedule-detail">📝 ${s.notes}</div>` : ''}</div><div class="schedule-actions">${s.status !== 'completed' ? `<button class="btn btn-warning btn-sm" onclick="updateScheduleStatus('${s.id}','in-progress')">▶️ Mulai</button>` : ''}${s.status === 'scheduled' ? `<button class="btn btn-danger btn-sm" onclick="deleteSchedule('${s.id}')">🗑️ Hapus</button>` : ''}</div></div>`;
    }).join('');
...
window.downloadPDF = async function(docId) {
  try {
    ...
    pdf.setFontSize(12); pdf.setFont(undefined, 'bold'); pdf.text('HASIL SUPERVISI PEMBELAJARAN', pageWidth / 2, y, { align: 'center' }); y += 6; 
    pdf.setFontSize(10); pdf.text('KURIKULUM BERBASIS CINTA (KBC)', pageWidth / 2, y, { align: 'center' }); y += 8;
    ...
    pdf.text(`Instrumen: ${data.instrumentName || '-'}`, margin, y); y += 8;
    ...
    if (components.length > 0) {
      ...
      pdf.setFillColor(76, 175, 80); pdf.setTextColor(255, 255, 255); pdf.setDrawColor(0, 0, 0); pdf.setLineWidth(0.2); 
      ...
      y += 6; pdf.setTextColor(0, 0, 0); pdf.setFont(undefined, 'normal');
      components.forEach((comp, index) => {
        const score = data.scores[`comp_${index}`] || 0; const rowHeight = 6; 
        if (y > 250) { 
          pdf.addPage(); y = 12; pdf.setFillColor(76, 175, 80); pdf.setTextColor(255, 255, 255); pdf.setDrawColor(0, 0, 0); pdf.setLineWidth(0.2); 
          for (let i = 0; i < headers.length; i++) { 
            const xPos = margin + colWidths.slice(0, i).reduce((a, b) => a + b, 0); 
            pdf.rect(xPos, y, colWidths[i], 6, 'FD'); pdf.text(headers[i], xPos + colWidths[i] / 2, y + 4, { align: 'center' }); 
          } 
        ...
        pdf.setDrawColor(0, 0, 0); pdf.setLineWidth(0.2); const totalWidth = colWidths.reduce((a, b) => a + b, 0); pdf.rect(margin, y, totalWidth, rowHeight, 'S');
        pdf.text(String(index + 1), margin + 3, y + 4); const compText = pdf.splitTextToSize(comp.name, colWidths[1] - 2); pdf.text(compText[0], margin + colWidths[0] + 1, y + 4);
        const boxSize = 3.5; 
        [col1X, col2X, col3X, col4X].forEach((cx, idx) => { 
          const value = idx + 1; 
          if (score === value) { 
            pdf.setFillColor(76, 175, 80); pdf.rect(cx - boxSize/2, y + 1.2, boxSize, boxSize, 'FD'); pdf.setDrawColor(255, 255, 255); pdf.setLineWidth(0.5); 
            pdf.line(cx - 1.2, y + 3, cx - 0.2, y + 4.2); pdf.line(cx - 0.2, y + 4.2, cx + 1.2, y + 1.8); pdf.setDrawColor(0, 0, 0); pdf.setLineWidth(0.2); 
          } else { pdf.setFillColor(255, 255, 255); pdf.rect(cx - boxSize/2, y + 1.2, boxSize, boxSize, 'FD'); } 
        });
        pdf.text(`${score}/4`, scoreX, y + 4, { align: 'center' }); y += rowHeight;
      });
      y += 2; pdf.setDrawColor(0, 0, 0); pdf.setLineWidth(0.2); pdf.setFillColor(200, 230, 201); 
      const footerWidth = colWidths.reduce((a, b) => a + b, 0); pdf.rect(margin, y, footerWidth, 6, 'FD'); pdf.setFont(undefined, 'bold'); pdf.text('Jumlah', margin + colWidths[0] + colWidths[1] - 2, y + 4, { align: 'right' });
      ...
      pdf.text(String(count1), col1X, y + 4, { align: 'center' }); pdf.text(String(count2), col2X, y + 4, { align: 'center' }); pdf.text(String(count3), col3X, y + 4, { align: 'center' }); pdf.text(String(count4), col4X, y + 4, { align: 'center' }); pdf.text(`${data.totalScore}/${data.maxScore}`, scoreX, y + 4, { align: 'center' });
      y += 6; pdf.setFillColor(165, 214, 167); pdf.rect(margin, y, footerWidth, 6, 'FD'); pdf.text('Persentase Capaian', margin + colWidths[0] + colWidths[1] - 2, y + 4, { align: 'right' }); pdf.text(`${data.percentage}%`, margin + colWidths[0] + colWidths[1] + 5, y + 4);
      y += 6; pdf.setFillColor(76, 175, 80); pdf.setTextColor(255, 255, 255); pdf.rect(margin, y, footerWidth, 6, 'FD'); pdf.text('Predikat', margin + colWidths[0] + colWidths[1] - 2, y + 4, { align: 'right' }); pdf.text(data.predicate || '-', margin + colWidths[0] + colWidths[1] + 5, y + 4);
      y += 8; pdf.setTextColor(0, 0, 0); pdf.setFontSize(7); pdf.setFont(undefined, 'normal'); pdf.text('Keterangan:', margin, y); y += 3.5; pdf.text('• 91% - 100% = Sangat Baik', margin, y); y += 3.5; pdf.text('• 81% - 90% = Baik', margin, y); y += 3.5; pdf.text('• 71% - 80% = Cukup', margin, y); y += 3.5; pdf.text('• < 70% = Kurang', margin, y);
    }
    if (data.notes) { 
      y += 6; if (y > 240) { pdf.addPage(); y = 12; } 
      pdf.setFontSize(8); pdf.setFont(undefined, 'bold'); pdf.text('Catatan Khusus Hasil Supervisi:', margin, y); y += 4; 
      pdf.setFont(undefined, 'normal'); pdf.setFontSize(7); 
      const splitNotes = pdf.splitTextToSize(data.notes, pageWidth - 2 * margin); pdf.text(splitNotes, margin, y); y += splitNotes.length * 3.5 + 2; 
    }
    if (data.actionPlan) { 
      y += 4; if (y > 245) { pdf.addPage(); y = 12; } 
      pdf.setFontSize(8); pdf.setFont(undefined, 'bold'); pdf.text('Rencana Tindak Lanjut:', margin, y); y += 4; 
      pdf.setFont(undefined, 'normal'); pdf.setFontSize(7); 
      const splitPlan = pdf.splitTextToSize(data.actionPlan, pageWidth - 2 * margin); pdf.text(splitPlan, margin, y); y += splitPlan.length * 3.5 + 2; 
    }
    y += 6; if (y > 250) { pdf.addPage(); y = 12; } 
    const signY = y; const leftSignX = margin + 15; const rightSignX = pageWidth - margin - 55; 
    pdf.setFontSize(7); pdf.setFont(undefined, 'normal'); pdf.text('Bantaeng, ' + date.toLocaleDateString('id-ID'), rightSignX, signY); 
    pdf.text('Guru yang disupervisi', leftSignX, signY + 12); pdf.text('Supervisor', rightSignX, signY + 12); 
    pdf.line(leftSignX, signY + 24, leftSignX + 50, signY + 24); pdf.line(rightSignX, signY + 24, rightSignX + 50, signY + 24); 
    pdf.setFont(undefined, 'bold'); pdf.text(data.superviseeName, leftSignX, signY + 28); pdf.text(data.supervisorName, rightSignX, signY + 28); 
    if (data.superviseeNIP) { pdf.setFont(undefined, 'normal'); pdf.text(`NIP. ${data.superviseeNIP}`, leftSignX, signY + 32); }
    if (data.supervisorNIP) { pdf.setFont(undefined, 'normal'); pdf.text(`NIP. ${data.supervisorNIP}`, rightSignX, signY + 32); }
