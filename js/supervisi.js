...
// ══════════════════════════════════════════════
// FUNGSI DOWNLOAD PDF (DIOPTIMALKAN 1 HALAMAN)
// ══════════════════════════════════════════════
window.downloadPDF = async function(docId) {
  try {
    ...
    // 1. Header (Sangat Compact)
    pdf.setFontSize(12); pdf.setFont(undefined, 'bold'); 
    pdf.text('HASIL SUPERVISI PEMBELAJARAN', pageWidth / 2, y, { align: 'center' }); y += 5; 
    pdf.setFontSize(10); 
    pdf.text('KURIKULUM BERBASIS CINTA (KBC)', pageWidth / 2, y, { align: 'center' }); y += 6;
    
    // 2. Info (2 Kolom untuk hemat ruang)
    ...
    pdf.text(`Yang Disupervisi: ${data.superviseeName}`, midX, y); y += 4;
    
    pdf.text(`Madrasah: ${data.schoolName || 'MAN BANTAENG'}`, margin, y); 
    pdf.text(`Kelas/Semester: ${data.classSemester || '-'}`, midX, y); y += 4;
    
    pdf.text(`Supervisor: ${data.supervisorName}`, margin, y); 
    pdf.text(`Mata Pelajaran: ${data.subject || '-'}`, midX, y); y += 4;
    
    pdf.text(`Instrumen: ${data.instrumentName || '-'}`, margin, y); y += 6;
    
    // 3. Tabel Penilaian (Ultra Compact)
    if (components.length > 0) {
      pdf.setFontSize(7); pdf.setFont(undefined, 'bold'); 
      const colWidths = [6, 68, 10, 10, 10, 10, 14]; // Total 128mm
      ...
      for (let i = 0; i < headers.length; i++) { 
        ...
        pdf.rect(xPos, y, colWidths[i], 5, 'FD'); 
        pdf.text(headers[i], xPos + colWidths[i] / 2, y + 3.5, { align: 'center' }); 
      } 
      y += 5; pdf.setTextColor(0, 0, 0); pdf.setFont(undefined, 'normal');
      
      components.forEach((comp, index) => {
        const score = data.scores[`comp_${index}`] || 0; const rowHeight = 5; 
        ...
        const compText = pdf.splitTextToSize(comp.name, colWidths[1] - 2); pdf.text(compText[0], margin + colWidths[0] + 1, y + 3.5);
        const boxSize = 3; 
        [col1X, col2X, col3X, col4X].forEach((cx, idx) => { 
          const value = idx + 1; 
          if (score === value) { 
            pdf.setFillColor(76, 175, 80); pdf.rect(cx - boxSize/2, y + 1, boxSize, boxSize, 'FD'); 
            pdf.setDrawColor(255, 255, 255); pdf.setLineWidth(0.4); 
            pdf.line(cx - 1, y + 2.5, cx - 0.2, y + 3.5); pdf.line(cx - 0.2, y + 3.5, cx + 1, y + 1.5); 
            pdf.setDrawColor(0, 0, 0); pdf.setLineWidth(0.2); 
          } else { pdf.setFillColor(255, 255, 255); pdf.rect(cx - boxSize/2, y + 1, boxSize, boxSize, 'FD'); } 
        });
        pdf.text(`${score}/4`, scoreX, y + 3.5, { align: 'center' }); y += rowHeight;
      });
      
      // Footer Tabel
      y += 1; pdf.setDrawColor(0, 0, 0); pdf.setLineWidth(0.2); pdf.setFillColor(200, 230, 201); 
      const footerWidth = colWidths.reduce((a, b) => a + b, 0); pdf.rect(margin, y, footerWidth, 5, 'FD'); pdf.setFont(undefined, 'bold'); 
      pdf.text('Jumlah', margin + colWidths[0] + colWidths[1] - 2, y + 3.5, { align: 'right' });
      ...
      pdf.text(String(count1), col1X, y + 3.5, { align: 'center' }); pdf.text(String(count2), col2X, y + 3.5, { align: 'center' }); 
      pdf.text(String(count3), col3X, y + 3.5, { align: 'center' }); pdf.text(String(count4), col4X, y + 3.5, { align: 'center' }); 
      pdf.text(`${data.totalScore}/${data.maxScore}`, scoreX, y + 3.5, { align: 'center' }); y += 5; 
      
      pdf.setFillColor(165, 214, 167); pdf.rect(margin, y, footerWidth, 5, 'FD'); 
      pdf.text('Persentase', margin + colWidths[0] + colWidths[1] - 2, y + 3.5, { align: 'right' }); 
      pdf.text(`${data.percentage}%`, margin + colWidths[0] + colWidths[1] + 5, y + 3.5); y += 5; 
      
      pdf.setFillColor(76, 175, 80); pdf.setTextColor(255, 255, 255); pdf.rect(margin, y, footerWidth, 5, 'FD'); 
      pdf.text('Predikat', margin + colWidths[0] + colWidths[1] - 2, y + 3.5, { align: 'right' }); 
      pdf.text(data.predicate || '-', margin + colWidths[0] + colWidths[1] + 5, y + 3.5); y += 6; 
      
      pdf.setTextColor(0, 0, 0); pdf.setFontSize(6); pdf.setFont(undefined, 'normal'); 
      pdf.text('Ket: 91-100%=Sangat Baik | 81-90%=Baik | 71-80%=Cukup | <70%=Kurang', margin, y); y += 5;
    }
    
    // 4. Catatan & Tindak Lanjut (Side-by-Side agar hemat ruang vertikal)
    pdf.setFontSize(7); pdf.setFont(undefined, 'bold');
    const colLeft = margin;
    const colRight = pageWidth / 2 + 5;
    const colWidth = (pageWidth / 2) - 15;
    
    pdf.text('Catatan Khusus:', colLeft, y);
    pdf.text('Rencana Tindak Lanjut:', colRight, y); y += 1;
    
    pdf.setFont(undefined, 'normal');
    const notesText = data.notes ? data.notes : '-';
    const planText = data.actionPlan ? data.actionPlan : '-';
    
    const splitNotes = pdf.splitTextToSize(notesText, colWidth);
    const splitPlan = pdf.splitTextToSize(planText, colWidth);
    
    // Batasi maksimal 6 baris per kolom agar MUAT 1 HALAMAN
    const maxLines = 6;
    const limitedNotes = splitNotes.slice(0, maxLines);
    const limitedPlan = splitPlan.slice(0, maxLines);
    
    pdf.text(limitedNotes, colLeft, y);
    pdf.text(limitedPlan, colRight, y);
    y += Math.max(limitedNotes.length, limitedPlan.length) * 3 + 5;
    
    // 5. Tanda Tangan (Dipaksa di bagian bawah halaman A4)
    const signY = 265; // Posisi Y tetap di bawah (A4 tinggi 297mm)
    ...
    pdf.text('Guru yang disupervisi', leftSignX, signY + 8); 
    pdf.text('Supervisor', rightSignX, signY + 8); 
    
    pdf.line(leftSignX, signY + 20, leftSignX + 50, signY + 20); 
    pdf.line(rightSignX, signY + 20, rightSignX + 50, signY + 20); 
    
    pdf.setFont(undefined, 'bold'); 
    pdf.text(data.superviseeName, leftSignX, signY + 24); 
    pdf.text(data.supervisorName, rightSignX, signY + 24); 
    
    if (data.superviseeNIP) { pdf.setFont(undefined, 'normal'); pdf.text(`NIP. ${data.superviseeNIP}`, leftSignX, signY + 28); }
    if (data.supervisorNIP) { pdf.setFont(undefined, 'normal'); pdf.text(`NIP. ${data.supervisorNIP}`, rightSignX, signY + 28); }
